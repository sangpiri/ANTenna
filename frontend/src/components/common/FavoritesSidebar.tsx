...
function formatTradingValue(value: number | null | undefined, market: Market): string {
  if (!value) return '-';
  if (market === 'kr') {
    if (value >= 1e12) {
...
function formatClose(value: number | null | undefined, market: Market): string {
...
function FavoritesSidebar({ isOpen, onClose, onSelectStock, market }: FavoritesSidebarProps) {
  ...
  const { data: prices = {} } = useQuery({
    queryKey: ['user', 'favorites', 'prices', currentFolderId, market],
    queryFn: () => userApi.getFavoritePrices(
      favorites.map(f => ({ code: f.code, market: f.market }))
    ),
    enabled: favorites.length > 0,
  });

  // 정렬 상태: null=기본, 'close'/'change_rate'/'trading_value' + 'asc'/'desc'
  ...
  const sortedFavorites = useMemo(() => {
    if (!sortKey) return favorites;
    // 문자열 정렬 (종목명, 코드)
    if (sortKey === 'name' || sortKey === 'code') {
      return [...favorites].sort((a, b) => {
        const va = sortKey === 'name' ? a.name : a.code;
        const vb = sortKey === 'name' ? b.name : b.code;
        const cmp = va.localeCompare(vb, 'ko');
        return sortDir === 'asc' ? cmp : -cmp;
      });
    }
    // 숫자 정렬 (종가, 등락률, 거래대금)
    if (Object.keys(prices).length === 0) return favorites;
    return [...favorites].sort((a, b) => {
      const pa = prices[a.code]?.[sortKey as keyof typeof prices[string]] ?? null;
      const pb = prices[b.code]?.[sortKey as keyof typeof prices[string]] ?? null;
      if (pa == null && pb == null) return 0;
      if (pa == null) return 1;
      if (pb == null) return -1;
      return sortDir === 'desc' ? (pb as number) - (pa as number) : (pa as number) - (pb as number);
    });
  }, [favorites, prices, sortKey, sortDir]);
  ...
  return (
    <>
      ...
      {currentFolderId && !movingFavorite && (
        <button
          onClick={handleBack}
          className="flex items-center gap-2 px-4 py-2 text-sm text-green-400 hover:bg-gray-700 border-b border-gray-700"
        >
          ...
          <div className="p-2 border-b border-gray-700">
            {isCreatingFolder ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm text-green-400 hover:bg-gray-700 rounded"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {currentFolderId && !movingFavorite && (
        <div className="flex-1 overflow-y-auto">
          {favorites.length === 0 ? (
            ...
            {/* 정렬 칩 */}
            {(
              <div className="flex gap-1.5 px-3 py-2 border-b border-gray-700 flex-wrap">
                {[
                  { key: 'name', label: '종목명' },
                  { key: 'code', label: market === 'kr' ? '코드' : '티커' },
                  { key: 'close', label: '종가' },
                  ...
                  >
                    ...
                    {prices[fav.code] && (
                      <div className="flex-shrink-0 text-right group-hover:hidden">
                        <div className="text-sm font-medium">{formatClose(prices[fav.code].close, market)}</div>
                        <div className="flex items-center gap-1.5 justify-end text-xs">
                          <span className={
                            (prices[fav.code].change_rate ?? 0) > 0 ? 'text-red-400' :
                            (prices[fav.code].change_rate ?? 0) < 0 ? 'text-blue-400' : 'text-gray-400'
                          }>
                            {(prices[fav.code].change_rate ?? 0) > 0 ? '+' : ''}{(prices[fav.code].change_rate ?? 0).toFixed(2)}%
                          </span>
                          <span className="text-gray-500">{formatTradingValue(prices[fav.code].trading_value, market)}</span>
                        </div>
                      </div>
                    )}
