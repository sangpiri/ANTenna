...
import KrGapAnalysisView from './KrGapAnalysisView';
import KrIpoListView from './KrIpoListView';
...
function KrStockView({ selectedFavoriteStock, onClearFavoriteStock, queryMode }: KrStockViewProps) {
  const [selectedDate, setSelectedDate] = useState<string | null>(null);
  const [category, setCategory] = useState<KrCategory>('trading_value');
  const [showCalendar, setShowCalendar] = useState(false);
  const [viewMode, setViewMode] = useState<ViewMode>('table');
  const [advancedTab, setAdvancedTab] = useState<'gap' | 'gap-down' | 'ipo'>('gap');
  ...
  const currentDate = selectedDate || datesInfo?.dates[0];
      ...
      {queryMode === 'advanced' && datesInfo && (
        <div className="flex-1 min-h-0 flex flex-col">
          ...
          <div className="flex gap-1 mb-2">
            <button
              onClick={() => setAdvancedTab('gap')}
              className={`px-2 sm:px-4 py-1.5 rounded text-xs sm:text-sm ${advancedTab === 'gap' ? 'bg-green-700 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            >
              갭 상승 조회
            </button>
            <button
              onClick={() => setAdvancedTab('gap-down')}
              className={`px-2 sm:px-4 py-1.5 rounded text-xs sm:text-sm ${advancedTab === 'gap-down' ? 'bg-green-700 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            >
              갭 하락 후 반등 조회
            </button>
            <button
              onClick={() => setAdvancedTab('ipo')}
              className={`px-2 sm:px-4 py-1.5 rounded text-xs sm:text-sm ${advancedTab === 'ipo' ? 'bg-green-700 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            >
              신규 상장 조회
            </button>
          </div>
