...
type SortKey = '날짜' | '종목코드' | '종목명' | '종가' | '등락률' | '거래대금' | null;
...
interface KrGapAnalysisViewProps {
...
function KrGapAnalysisView({ onStockClick, availableDates, minYear, maxYear, initialYear, initialMonth, direction = 'up' }: KrGapAnalysisViewProps) {
  ...
  const { data: tickerResults = [] } = useQuery({
    queryKey: ['kr', 'search', tickerQuery],
    queryFn: () => krApi.search(tickerQuery, 10),
  ...
  const { data: results = [], isLoading } = useQuery({
    queryKey: ['kr', 'gap-analysis', searchParams],
    queryFn: () =>
      krApi.getGapAnalysis(
  ...
  const sortedResults = useMemo(() => {
    ...
    let filtered: GapAnalysisResult[] = results;
  ...
  }, [results, sortKey, sortDir, maFilter, highFilter]);
          ...
          <>
            ...
            <div className="flex items-center gap-x-2 gap-y-1 px-2 sm:px-4 py-1.5 sm:py-2 bg-gray-700/50 border-b border-gray-700 flex-wrap">
              ...
              <span className="text-xs sm:text-sm text-gray-400 ml-auto">
                ...
                {(maFilter !== 'all' || highFilter !== 'all') && ` (전체 ${results.length.toLocaleString()}개)`}
              </span>
            </div>
...
export default KrGapAnalysisView;
