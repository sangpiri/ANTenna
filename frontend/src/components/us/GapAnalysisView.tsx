...
type HighFilter = 'all' | 'high' | 'not_high';
...
const HIGH_FILTER_OPTIONS: { value: HighFilter; label: string }[] = [
  { value: 'all', label: '전체' },
  { value: 'high', label: '52주 신고가' },
  { value: 'not_high', label: '52주 신고가 외' },
];
...
const MIN_RATE_OPTIONS = [3, 5, 7, 10];
const MAX_RATE_OPTIONS = [3, 5, 7, 10, 30, 50, 100, 99999];
...
function GapAnalysisView({ onStockClick, availableDates, minYear, maxYear, initialYear, initialMonth, direction = 'up' }: GapAnalysisViewProps) {
  // 필터 상태
  ...
  const applyPreset = (days: number) => {
    ...
    const latestDate = availableDates[0];                             // DESC → [0]이 최신
    const earliestDate = availableDates[availableDates.length - 1];  // DESC → [length-1]이 가장 오래된
  ...
  const sortedResults = useMemo(() => {
    ...
    // 52주 신고가 필터링
    if (highFilter === 'high') {
      filtered = filtered.filter(item => item.is_52week_high);
    } else if (highFilter === 'not_high') {
      filtered = filtered.filter(item => !item.is_52week_high);
    }
  ...
  }, [results, sortKey, sortDir, priceRange, maFilter, highFilter]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="bg-gray-800 rounded-lg mb-4">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <span className="text-xs sm:text-sm bg-green-700 text-white px-2 py-1 rounded w-full sm:w-auto">기준</span>
          <select
            value={basePrice}
          ...
          <span className="hidden sm:inline text-gray-600 mx-1">|</span>
          <div className="w-full sm:hidden" />
          <span className="text-xs sm:text-sm text-gray-400">{direction === 'down' ? '하락률:' : '상승률:'}</span>
          <select
            value={minRate}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs sm:text-sm bg-green-700 text-white px-2 py-1 rounded w-full sm:w-auto">추가 기준</span>
          <select
            value={extraBase}
            onChange={(e) => {
          ...
          <span className="text-xs sm:text-sm bg-blue-400 text-white px-2 py-1 rounded w-full sm:w-auto">종목</span>
          <div ref={tickerContainerRef} className="relative">
            <input
              ...
              placeholder="티커/종목명"
              className="bg-gray-700 border border-gray-600 rounded px-2 sm:px-3 py-1 text-xs sm:text-sm w-32 sm:w-40 placeholder-gray-500 pr-7"
            />
            {tickerQuery && (
              <button
          ...
          <span className="text-xs sm:text-sm bg-green-700 text-white px-2 py-1 rounded w-full sm:w-auto">세부 기준</span>
          <select
            value={detailBase}
            onChange={(e) => {
      ...
      <div className="flex-1 min-h-0 bg-gray-800 rounded-lg overflow-hidden flex flex-col">
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-xs sm:text-sm text-gray-400">기간과 기준을 선택하고 검색 버튼을 클릭하세요.</div>
          </div>
        ) : results.length === 0 ? (
          <div className="flex items-center justify-center h-full">
          ...
          <>
            {/* 가격대 및 MA240 필터 바 */}
            <div className="flex items-center gap-x-2 gap-y-1 px-2 sm:px-4 py-1.5 sm:py-2 bg-gray-700/50 border-b border-gray-700 flex-wrap">
              <span className="text-xs sm:text-sm text-gray-400">가격대:</span>
              <select
                value={priceRange}
              ...
              <span className="text-xs sm:text-sm text-gray-400">배열:</span>
              <select
                value={maFilter}
              ...
              <span className="text-xs sm:text-sm text-gray-400">신고가:</span>
              <select
                value={highFilter}
                onChange={(e) => setHighFilter(e.target.value as HighFilter)}
                className="bg-gray-700 border border-gray-600 rounded px-2 sm:px-3 py-1 text-xs sm:text-sm"
              >
                {HIGH_FILTER_OPTIONS.map(opt => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>
              ...
              <span className="text-xs sm:text-sm text-gray-400 ml-auto">
                ...
                {(priceRange !== 'all' || maFilter !== 'all' || highFilter !== 'all') && ` (전체 ${results.length.toLocaleString()}개)`}
              </span>
            </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-700 sticky top-0">
                <tr>
                  ...
                  <th
                    ...
                    className="px-2 sm:px-4 py-2 sm:py-3 text-right text-xs sm:text-sm font-medium cursor-pointer hover:bg-gray-600"
                  >
                    <span className="sm:hidden"><span className="block whitespace-nowrap">거래</span><span className="block whitespace-nowrap">대금</span></span>
              ...
              <tbody>
                {sortedResults.map((item, idx) => (
                  ...
                  >
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm">{item.날짜}</td>
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-mono text-yellow-400">{item.티커}</td>
                    ...
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-right">${item.종가.toFixed(2)}</td>
                    ...
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-right text-gray-400">
