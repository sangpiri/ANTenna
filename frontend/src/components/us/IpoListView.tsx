...
type SortKey = '날짜' | '티커' | '종목명' | '시가' | '종가' | '등락률' | '거래대금' | null;
...
function IpoListView({ onStockClick, availableDates, minYear, maxYear, initialYear, initialMonth }: IpoListViewProps) {
  ...
  const { data: results = [], isLoading } = useQuery<NewListingResult[]>({
    queryKey: ['us', 'new-listings', searchParams],
    queryFn: () => usApi.getNewListings(searchParams!.startDate, searchParams!.endDate),
  ...
  }, [results, sortKey, sortDir, priceRange]);
          ...
          <>
            ...
            <div className="flex items-center gap-x-2 gap-y-1 px-2 sm:px-4 py-1.5 sm:py-2 bg-gray-700/50 border-b border-gray-700">
              ...
              <span className="text-xs sm:text-sm text-gray-400 ml-auto">
                ...
                {priceRange !== 'all' && ` (전체 ${results.length.toLocaleString()}개)`}
              </span>
            </div>
              ...
              <tbody>
                {sortedResults.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-right">${item.시가.toFixed(2)}</td>
